...
const TAG_START_PATTERN = /[a-zA-Z0-9.:_-]/;
  ...
  strings.forEach((str, i) => {
    ...
    while (cursor < str.length) {
      ...
      if (char === '<' && !inQuotes) {
        ...
        const isOpenTag = nextChar && TAG_START_PATTERN.test(nextChar); // <div, <Component, <my-component, <ns:tag
      ...
      if (tagDepth > 0 && !inQuotes) {
        ...
        if (IDENTIFIER_PATTERN.test(char)) {
          const startIdx = cursor;
          while (cursor < str.length && IDENTIFIER_PATTERN.test(str[cursor])) {
            cursor++;
          }
